...
{
    public class QuestionBank1Repository : IQuestionBank1Repository
    {
        DBEntities2 db;

        public QuestionBank1Repository()
        {
            db = new DBEntities2();
        }
        ...
        public IEnumerable<questionbank> Get(int organizationUserId)
        {
            var result = from t in db.questionbank
                         where 
                         (
                            from t1 in db.establish_questionbank_between_organizationuser_and_questionbank
                            where t1.OrganizationUserID==organizationUserId
                            select t1.QuestionBankID
                         ).Contains(t.ID)
                         select t;
